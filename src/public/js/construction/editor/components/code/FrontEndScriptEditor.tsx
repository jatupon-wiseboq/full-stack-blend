...
Object.assign(ExtendedDefaultProps, {
  ...
  watchingExtensionNames: ['autoGeneratedCodeForRenderMethod', 'autoGeneratedCodeForMergingRenderMethod', 'isInheritingComponent', 'editing']
});

class FrontEndScriptEditor extends Base<Props, State> {
  protected static defaultProps: Props = ExtendedDefaultProps;
  private recentRange: any = {};
  ...
  public update(properties: any) {
    ...
    if (this.state.extensionValues['isInheritingComponent']) {
    ...
    } else if (['site', 'components', 'popups'].indexOf(this.state.extensionValues['editing']) != -1) {
      let code, mapping, editing;
      if (this.state.attributeValues['internal-fsb-react-mode']) {
        ...
        editing = info['internal-fsb-guid'];
      } else {
        let info = this.state.attributeValues;
        info.autoGeneratedCodeForRenderMethod = this.state.extensionValues['autoGeneratedCodeForRenderMethod'];
        info.autoGeneratedCodeForMergingRenderMethod = this.state.extensionValues['autoGeneratedCodeForMergingRenderMethod'];
      ...
      if (this.editing != editing) {
        this.recentRange[this.editing] = editor.selection.getRange();
      ...
      if (this.state.value !== code) {
        ...
        if (this.recentRange[this.editing] !== undefined) {
          editor.scrollToLine(this.recentRange[this.editing].start.row, true, true, function () {});
          editor.moveCursorTo(this.recentRange[this.editing].start.row, this.recentRange[this.editing].start.column);
          editor.selection.setRange(this.recentRange[this.editing]);
        }
