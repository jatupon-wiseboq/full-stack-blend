...
var BackEndDOMHelper = {
	generateBackEndCode: function(key: string, container: HTMLElement=document.body) {
		let info = {};
		Object.assign(info, InternalProjectSettings);
		
		info['autoGeneratedCodeForMergingBackEndScript'] = BackEndDOMHelper.generateCodeForMergingSection(key, container);
	...
	generateCodeForMergingSection: function(key: string, element: HTMLElement) {
  ...
  recursiveGenerateCodeForMergingSection: function(key: string, element: HTMLElement, executions: string[], lines: string[]) {
    ...
    if (element && element.tagName) {
      ...
      for (let child of children) {
        BackEndDOMHelper.recursiveGenerateCodeForMergingSection(key, child, executions, lines);
      }
    }
  }
}
