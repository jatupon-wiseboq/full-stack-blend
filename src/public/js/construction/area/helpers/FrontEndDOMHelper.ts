...
var FrontEndDOMHelper = {
  ...
  generateFrontEndCode: function(root: HTMLElement, container: HTMLElement=HTMLHelper.getElementByAttributeNameAndValue("internal-fsb-guid", "0")) {
    // Document Level
    // 
    let generatedRenderMethodRootResult = FrontEndDOMHelper.generateCodeForReactRenderMethod(root, container);
    let generatedMergingSectionRootResult = FrontEndDOMHelper.generateCodeForMergingSection(root, container);
    ...
    FrontEndDOMHelper.recursiveGenerateCodeForPage(root, container, '    ', executions, lines);
...
${rootScript}`;
    ...
    let links = HTMLHelper.getElementsByAttribute('internal-fsb-link', root);
  ...
  recursiveGenerateCodeForPage: function(root: HTMLElement, element: HTMLElement, indent: string, executions: string[], lines: string[], isFirstElement: boolean=true) {
    ...
    if (element && element.tagName) {
      if (!isFirstElement && HTMLHelper.getAttribute(element, 'internal-fsb-react-mode') && !HTMLHelper.getAttribute(element, 'internal-fsb-inheriting') && !HTMLHelper.hasClass(element, 'internal-fsb-plug')) {
        ...
        _info.autoGeneratedCodeForRenderMethod = FrontEndDOMHelper.generateCodeForReactRenderMethod(root, element);
        _info.autoGeneratedCodeForMergingRenderMethod = FrontEndDOMHelper.generateCodeForMergingSection(root, element);
      ...
      for (let child of children) {
        FrontEndDOMHelper.recursiveGenerateCodeForPage(root, child, indent, executions, lines, false);
      }
    }
  },
  generateCodeForReactRenderMethod: function(root: HTMLElement, element: HTMLElement) {
    ...
    if (EditorHelper.hasParentReactComponent(element)) {
      ...
      executions.push(`    function ready(a){"loading"!=document.readyState?a(new Event('ready')):document.addEventListener?document.addEventListener("DOMContentLoaded",a):(document.onreadystatechange=function(e){"complete"==document.readyState&&a(e)})};
        `);
      FrontEndDOMHelper.recursiveGenerateCodeForReactRenderMethod(root, element, '      ', executions, lines);
    } else {
      executions.push(`TestHelper.identify();`);
      executions.push(`function ready(a){"loading"!=document.readyState?a(new Event('ready')):document.addEventListener?document.addEventListener("DOMContentLoaded",a):(document.onreadystatechange=function(e){"complete"==document.readyState&&a(e)})};
        `);
      FrontEndDOMHelper.recursiveGenerateCodeForFallbackRendering(root, element, '    ', executions, lines);
    }
  ...
  recursiveGenerateCodeForReactRenderMethod: function(root: HTMLElement, element: HTMLElement, indent: string, executions: string[], lines: string[], isFirstElement: boolean=true, cumulatedDotNotation: string="", dotNotationChar: string='i', forwardAttributes: string[]=null, context: any={}) {
    if (HTMLHelper.hasClass(element, 'internal-fsb-accessory')) return;
    ...
    if (element) {
      if (!element.tagName) {
        ...
        if (submitControls) {
          ...
          splited = splited.filter(submitControl => !!HTMLHelper.getElementByAttributeNameAndValue('internal-fsb-guid', submitControl, root));
          ...
          for (let submitControl of splited) {
            let control = HTMLHelper.getElementByAttributeNameAndValue('internal-fsb-guid', submitControl, root);
            if (control) {
        ...
        if (!reactMode || isFirstElement) {
          ...
          if (!dangerouslySetInnerHTML) {
            ...
            for (let child of children) {
              FrontEndDOMHelper.recursiveGenerateCodeForReactRenderMethod(root, child, indent + '  ', executions, lines, false, cumulatedDotNotation, dotNotationChar, _forwardAttributes, (reactMode && !isFirstElement) ? {} : context);
            }
          } else {
            lines.push(composed);
          }
  ...
  recursiveGenerateCodeForFallbackRendering: function(root: HTMLElement, element: HTMLElement, indent: string, executions: string[], lines: string[], isFirstElement: boolean=true) {
    if (HTMLHelper.hasClass(element, 'internal-fsb-accessory')) return;
    if (HTMLHelper.hasClass(element, 'internal-fsb-plug')) return;
    ...
    if (element) {
      if (!element.tagName) {
        ...
        if (!reactMode || isFirstElement) {
          ...
          for (let child of children) {
            FrontEndDOMHelper.recursiveGenerateCodeForFallbackRendering(root, child, indent + '  ', executions, lines, false);
          }
        }
      }
    }
  },
  generateCodeForMergingSection: function(root: HTMLElement, element: HTMLElement) {
    let executions: string[] = [];
    let lines: string[] = [];
    FrontEndDOMHelper.recursiveGenerateCodeForMergingSection(root, element, executions, lines, true, EditorHelper.hasParentReactComponent(element));
    
    return [executions.join('\n'), lines.join('\n')];
  },
  recursiveGenerateCodeForMergingSection: function(root: HTMLElement, element: HTMLElement, executions: string[], lines: string[], isFirstElement: boolean=true, hasParentReactComponent: boolean=true) {
    if (HTMLHelper.hasClass(element, 'internal-fsb-accessory')) return;
    ...
    if (element && element.tagName) {
      ...
      let children = [...element.childNodes];
      for (let child of children) {
        FrontEndDOMHelper.recursiveGenerateCodeForMergingSection(root, child, executions, lines, false, hasParentReactComponent);
      }
    }
  },
