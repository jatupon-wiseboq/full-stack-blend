      ...
      if (!element.tagName) {
        ...
        if (submitControls) {
          executions.push(`DataManipulationHelper.register(${JSON.stringify(reactClassComposingInfoGUID)}, ${JSON.stringify(submitType)}, ${JSON.stringify(submitControls && submitControls.split(' ') || [])}, {initClass: ${JSON.stringify(reactClassForPopup)}})`);
        	...
        	attributes.push(`onClick={((event) => { window.internalFsbSubmit('${reactClassComposingInfoGUID}', '${cumulatedDotNotation + reactData}', event, ((results: any) => { this.manipulate('${reactClassComposingInfoGUID}', '${cumulatedDotNotation + reactData}', results); }).bind(this)); }).bind(this)}`);
        }
