...
${CLASS_END_BEGIN}`);
        ...
        code = code.replace('// Auto[Merging]--->\n', '// Auto[Merging]--->' + (info['autoGeneratedCodeForMergingSection'] || '\n'));
    ...
    ${LOAD_END}${code.split(LOAD_END)[1]}`;
        ...
        code = `${code.split(RENDER_BEGIN)[0]}${RENDER_BEGIN}${info['autoGeneratedCodeForRenderMethod']}${RENDER_END}${code.split(RENDER_END)[1]}`;
        
        if (previewReactClassName) {
        		code = `${code}
...
// <---Auto[Export]`;
    ...
    generateMergingCode: (info: any, removeAutoGeneratingWarning: boolean=false) => {
        let code = '';
        let functionNameMapping = {};
...
export {CodeGenerationHelper, DEFAULTS};